...
<script lang="ts">
	...
	const OFFICIAL_DONATION_URL = "https://donations.filetools.app";
	...
	onMount(async () => {
		if (!isOfficial) {
			log(
				["about", "donate"],
				"donations are being sent to an unofficial FileTools instance - PUB_DONATION_URL and/or PUB_STRIPE_KEY have been changed.",
			);
		} else {
			log(
				["about", "donate"],
				"donations are being sent to the official FileTools instance.",
			);
		}
	});
