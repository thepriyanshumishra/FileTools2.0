import FileToolsDErrorComponent from "$lib/components/functional/FileToolsDError.svelte";
...
export class FileToolsDConverter extends Converter {
	public name = "filetoolsd";
	...
	public async convert(input: VertFile, to: string): Promise<VertFile> {
		...
		return new Promise((resolve, reject) => {
			...
			ws.onmessage = async (e) => {
				const msg: FileToolsDMessage = JSON.parse(e.data);
				...
				switch (msg.type) {
					...
					case "error": {
						...
						reject({
							component: FileToolsDErrorComponent,
							additional: {
								jobId: uploadRes.id,
								auth: uploadRes.auth,
								from: input.from,
