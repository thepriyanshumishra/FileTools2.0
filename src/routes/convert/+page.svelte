<script lang="ts">
	...
	$effect(() => {
		...
		files.files.forEach((file) => {
			...
			const isVideo = converter.name === "filetoolsd";
	...
	$effect(() => {
		...
		if (files.files.length) {
			...
			if (uniqueTypes.size === 1) {
				...
				else if (onlyType === "filetoolsd") type = "red";
...
{#snippet fileItem(file: VertFile, index: number)}
	...
	<Panel class="p-5 flex flex-col min-w-0 gap-4 relative">
		...
		{#if !currentConverter}
			{#if file.name.startsWith("filetoolsd")}
				...
				>
					...
					<p class="font-normal">
						{m["convert.errors.filetoolsd_server"]()}
					</p>
				</div>
			{:else}
				<div
		...
		{:else}
			...
			{:else if isVideo && !$filetoolsdLoaded && !isAudio && !isImage && !isDocument}
				<div
					class="h-full flex flex-col text-center justify-center text-failure"
				>
					<p class="font-body font-bold">
					...
					<p class="font-normal">
						{m["convert.errors.filetoolsd_not_found"]()}
					</p>
				</div>
			{:else}
